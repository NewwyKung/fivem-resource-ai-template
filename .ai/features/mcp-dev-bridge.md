...
## Files
- Config: `.mcp-config.json.example` (copy to repo root as `.mcp-config.json`); `.cursor/mcp.json`; `package.json` (`mcp` script, `@modelcontextprotocol/sdk`, `zod`, `tsx`).
- Modules: `mcp_dev_bridge/fxmanifest.lua`, `mcp_dev_bridge/server/dev_bridge.lua`, `dev_bridge_logger.lua`.
...
- Callbacks: none.
- Exports: `mcp_getLogs(lines, levelFilter)` from any resource carrying `dev_bridge_logger.lua`.
- State bags: none.
- HTTP: `POST /mcp/restart`, `POST /mcp/agent/action`, `GET /mcp/logs` on the `mcp_dev_bridge` resource, Bearer-token authenticated.
